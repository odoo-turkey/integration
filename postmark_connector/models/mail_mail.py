...
class MailMail(models.Model):
    ...
    def _postprocess_sent_message(
        ...
        for mail in self:
            ...
            # TODO SAMET: add "if sale_order:"
            new_state = (
                "02_sent"
                if mail.mail_message_id.postmark_api_state == "sent"
                else "011_email_error"
            )
        ...
        if self.mail_server_id and "postmark" in self.mail_server_id.smtp_host:
            ...
            for mail_id in self.ids:
                ...
                try:
                    ...
                    if (
                        res
                    ):  # mail has been sent at least once to Postmark, no major exception occured
                        mail.write(
                            {
                                "state": "sent",
                                "message_id": mail.message_id,
