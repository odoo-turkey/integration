...
class GarantiConnector:
    ...
    def __init__(self, provider, tx, amount, currency, card_args=None,
                 client_ip=None):
        self.endpoint = provider._garanti_get_api_url()
        self.provider = provider
        self.tx = tx
        self.amount = int(round(amount * 100, 0))  # Garanti API expects amount in kuruş.
    ...
    def _garanti_parse_response_html(self, response):
        ...
        if error_msg:
            if error_msg['value'] == 'Not Authenticated':
                raise ValidationError(_('Payment failed.'
                                        ' Garanti: Card is not authenticated.')
                                      )
            raise ValidationError(
                _('Garanti Sanal Pos Error: %s') % error_msg['value'])
    ...
    def _garanti_make_payment_request(self):
        ...
        try:
            resp = requests.post(self.provider._garanti_get_api_url(),
                                 params=vals,
                                 timeout=10)
    ...
    def _garanti_compute_security_data(self):
        return sha1((self.provider.garanti_prov_password +
    ...
    def _garanti_create_secure3d_hash(self):
        ...
        """
        hash_strings = (
                str(self.provider.garanti_terminal_id) +  # terminalID
                str(self.tx.reference) +  # orderid
                str(self.amount) +  # txnamount
                str(self.provider._garanti_get_return_url()) +  # successurl
                str(self.provider._garanti_get_return_url()) +  # errorurl
                'sales' +  # txntype
                # "" +  # txninstallmentcount
                str(self.provider.garanti_store_key) +  # storekey
                str(self._garanti_compute_security_data())  # securitydata
        )
        self.tx.garanti_secure3d_hash = sha1(
            hash_strings.encode('utf-8')).hexdigest().upper()
    ...
    def _garanti_compute_callback_hash_data(self):
        ...
        """
        hash_data = (
                str(self.notification_data.get('oid')) +  # orderid
                str(self.notification_data.get('clientid')) +  # clientid
                str(self.notification_data.get('txnamount')) +  # txnamount
    ...
    def _garanti_terminal_node(self, gvps_request):
        ...
        etree.SubElement(terminal, 'ProvUserID').text = self.notification_data.get('terminalprovuserid')
        etree.SubElement(terminal, 'HashData').text = self._garanti_compute_callback_hash_data()
        etree.SubElement(terminal, 'UserID').text = self.notification_data.get('terminaluserid')
        etree.SubElement(terminal, 'ID').text = self.notification_data.get('clientid')
        etree.SubElement(terminal,'MerchantID').text = self.notification_data.get('terminalmerchantid')
        return True
    ...
    def _garanti_customer_node(self, gvps_request):
        ...
        etree.SubElement(customer, 'IPAddress').text = self.notification_data.get('customeripaddress')
        etree.SubElement(customer, 'EmailAddress').text = self.notification_data.get('customeremailaddress')
        return True
    ...
    def _garanti_transaction_node(self, gvps_request):
        ...
        etree.SubElement(transaction, 'Type').text = self.notification_data.get('txntype')
        etree.SubElement(transaction, 'InstallmentCnt').text = self.notification_data.get('txninstallmentcount')
        etree.SubElement(transaction, 'Amount').text = self.notification_data.get('txnamount')
        etree.SubElement(transaction, 'CurrencyCode').text = self.notification_data.get('txncurrencycode')
        ...
        etree.SubElement(secure3d, 'AuthenticationCode').text = self.notification_data.get('cavv')
        etree.SubElement(secure3d, 'SecurityLevel').text = self.notification_data.get('eci')
    ...
    def _garanti_create_callback_xml(self):
        ...
        return etree.tostring(gvps_request, encoding='UTF-8',
    ...
    def _garanti_payment_callback(self, notification_data):
        ...
        try:
            resp = requests.post(PROVISION_URL,
